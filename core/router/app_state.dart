...
class AppState extends ChangeNotifier {
  ...
  addWidget({required PageConfiguration page, required Widget child}) {
    ...
    appState.currentAction =
        PageAction(state: PageState.addWidget, page: page, widget: child);
  }
  ...
  removeManyPage(List<PageConfiguration> pageConfigs) {
    currentAction = PageAction(pages: pageConfigs, state: PageState.removeMany);
  }

  addAllPages(List<PageConfiguration> pageConfigs) {
    currentAction = PageAction(pages: pageConfigs, state: PageState.addAll);
  }

  replacefirstAndAddAnotherPage(
      PageConfiguration replacePage, List<PageConfiguration> pageConfigs) {
    currentAction = PageAction(
        page: replacePage,
        pages: pageConfigs,
        state: PageState.addAndReplaceAll);
  }

  removePageAndReplace(PageConfiguration pageConfigs) {
    removePage(pageConfigs);
    currentAction = PageAction(page: pageConfigs, state: PageState.addPage);
  }

  dynamic moveToBackScreen({dynamic data}) {
    currentAction = const PageAction(state: PageState.pop);
    if (waitToPop != null) {
      waitToPop!.complete(data);
      waitToPop = null;
    }
  }
